...
def plotPowerandCPU(vcodecs, allsummary, targets=[]):
    vcs = []
    powers= []
    power_confs = []
    cpus = []
    ...
    ax1.set_ylim([60,120])
    ax1.set_xlabel('Video Heights (Youtube)')
    ...
    plt.title('vEQ-benchmark - Summary results\n (Windows workstation, Youtube videos )\n')
    
    if not os.path.exists("plots"): 
        os.makedirs("plots")
        
    plt.savefig("plots/" + st + ".pdf")
    
    plt.show()       
    print time.time()-s

plotPowerandCPU(vheights, heightssummary, targets=['320','480','720','1080','1440','2160'])